...
export default function CheckoutPage() {
  ...
  const Field = ({ label, id, placeholder, type = 'text', req = true }: { label: string; id: keyof FormState; placeholder: string; type?: string; req?: boolean }) => (
    <div style={{ marginBottom: 20 }}>
      ...
      <input type={type} value={form[id]} onChange={(e) => set(id, e.target.value)} placeholder={placeholder}
        style={{ width: '100%', padding: '13px 16px', border: `1px solid ${errors[id] ? '#c0392b' : 'rgba(184,131,111,0.25)'}`, background: '#fff', fontFamily: "'Jost'", fontSize: 14, color: '#3a2a24', outline: 'none', transition: 'border-color 0.2s' }} />
  ...
  if (items.length === 0) {
    return (
      <main><Navbar />
        <div style={{ paddingTop: 72, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 24 }}>
          <p style={{ fontFamily: "'Cormorant Garamond'", fontSize: 28, color: '#9a7a74' }}>Votre panier est vide</p>
            ...
            {['Livraison', 'Confirmation'].map((s, i) => (
              <div key={s} style={{ display: 'flex', alignItems: 'center', flex: i === 0 ? 'none' : 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: i === 0 ? '#b8836f' : 'rgba(184,131,111,0.15)', border: i === 1 ? '2px solid rgba(184,131,111,0.3)' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Jost'", fontSize: 11, color: i === 0 ? '#fff' : '#b8836f' }}>{i + 1}</div>
            ...
            <div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <select value={form.wilaya} onChange={(e) => set('wilaya', e.target.value)}
                  style={{ width: '100%', padding: '13px 16px', border: `1px solid ${errors.wilaya ? '#c0392b' : 'rgba(184,131,111,0.25)'}`, background: '#fff', fontFamily: "'Jost'", fontSize: 14, color: form.wilaya ? '#3a2a24' : '#aaa', outline: 'none', cursor: 'pointer' }}>
              ...
              <div style={{ marginBottom: 28 }}>
                ...
                <textarea value={form.notes} onChange={(e) => set('notes', e.target.value)} placeholder="Instructions de livraison..."
                  style={{ width: '100%', padding: '13px 16px', height: 90, resize: 'vertical', border: '1px solid rgba(184,131,111,0.25)', background: '#fff', fontFamily: "'Jost'", fontSize: 14, color: '#3a2a24', outline: 'none' }} />
              </div>
              ...
              <div style={{ background: 'rgba(184,131,111,0.06)', border: '1px solid rgba(184,131,111,0.15)', padding: '20px 24px', marginBottom: 32 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ width: 20, height: 20, borderRadius: '50%', background: '#b8836f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <Btn primary onClick={placeOrder} disabled={loading} style={{ padding: '16px 60px', fontSize: 13 }}>
                {loading ? 'Traitement...' : 'Confirmer la Commande →'}
              </Btn>
            </div>

            {/* Order summary */}
            <div style={{ background: '#f5efe8', padding: '28px', border: '1px solid rgba(184,131,111,0.1)', position: 'sticky', top: 100 }}>
